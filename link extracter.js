;(function () {
    ...
    noOfSubreddits = document.querySelector("body > div.side > div:nth-child(4) > div > div > h1")?.innerText.split("(")[1].split(")")[0]
    ...
    function createLink() {
        for (let i = 1; i <= noOfSubreddits; i++) {
            let component = document.querySelector(
                `body > div.side > div:nth-child(4) > div > ul > li > div > div.clear > ul > li:nth-child(${i + 2}) > a`
            )
    ...
    console.log("--------------------------------OPEN THESE LINKS IN YOUR NEW ACCOUNT------------------------------------")
    ...
    console.log("------------------------------RIGHT CLICK AND COPY, DON'T CTRL+C AND CTRL+V-----------------------------")
